...
static void new_game(void)
{
    ...
    int draw_count = app.game.draw_count == 3 ? 3 : 1;
    solitaire_new(&app.game, seed);
    app.game.draw_count = draw_count;
    app.has_selection = FALSE;
    set_message("New Windows Solitaire-style Klondike deal. Tap stock or Draw.");
    update_ui();
}
...
static void draw_mode_cb(GtkWidget *widget, gpointer data)
{
    (void)widget;
    (void)data;

    app.game.draw_count = app.game.draw_count == 3 ? 1 : 3;
    app.has_selection = FALSE;
    set_message(app.game.draw_count == 3 ?
                "Draw 3 mode: waste shows up to 3, only top card moves." :
                "Draw 1 mode.");
    update_ui();
}
...
static void compute_layout(GtkAllocation *a)
{
    ...
    double card_w = (usable_w - gap * 6.0) / 7.0;
    ...
    if (card_w > 122.0) {
        card_w = 122.0;
        card_h = card_w * 1.38;
        margin = (a->width - (card_w * 7.0 + gap * 6.0)) / 2.0;
    }
    ...
    for (i = 0; i < SOL_FOUNDATION_COUNT; i++) {
        double x = margin + (3 + i) * (card_w + gap);
...
static void draw_card_face(cairo_t *cr, Rect r, SolCard card)
{
    ...
    Rect center_rank = { r.x, r.y + r.h * 0.16, r.w, r.h * 0.38 };
    ...
    draw_centered_text(cr, label, top_box, 21.0);
    set_card_ink(cr, card.suit);
    draw_centered_text(cr, solitaire_rank_label(card.rank), center_rank, 48.0);
    draw_suit_icon(cr, card.suit, r.x + r.w * 0.50, r.y + r.h * 0.71, MIN(r.w, r.h) * 0.24);

    cairo_save(cr);
    cairo_translate(cr, r.x + r.w - 5.0, r.y + r.h - 5.0);
    cairo_rotate(cr, G_PI);
    set_card_ink(cr, card.suit);
    draw_centered_text(cr, label, (Rect){ 0, 0, top_box.w, top_box.h }, 18.0);
    cairo_restore(cr);
}
...
static gboolean board_expose(GtkWidget *widget, GdkEventExpose *event, gpointer data)
{
    ...
    for (i = 0; i < SOL_TABLEAU_COUNT; i++) {
        ...
        int j;
        if (pile->count == 0)
            draw_empty_slot(cr, app.tableau_rects[i], "K");
...
static void handle_location(SolLocation loc)
{
    ...
    if (app.has_selection) {
        ...
        app.has_selection = FALSE;
        if (result == SOL_MOVE_INVALID) {
            if (loc.type == SOL_LOC_FOUNDATION || loc.type == SOL_LOC_TABLEAU) {
                set_message("That move is not legal in Klondike.");
            } else {
                app.selected = loc;
    ...
    if (loc.type == SOL_LOC_TABLEAU && app.game.tableau[loc.index].count == 0) {
        set_message("Select a king, then tap the empty tableau slot.");
        update_ui();
        return;
    }
    ...
    app.selected = loc;
    app.has_selection = TRUE;
    set_message("Card selected. Tap a tableau column or foundation.");
    update_ui();
}
...
static void update_ui(void)
{
    ...
    if (app.draw_mode_button)
        gtk_button_set_label(GTK_BUTTON(app.draw_mode_button), app.game.draw_count == 3 ? "Draw 3" : "Draw 1");
...
int main(int argc, char **argv)
{
    ...
    g_signal_connect(button, "clicked", G_CALLBACK(draw_cb), NULL);
    gtk_box_pack_start(GTK_BOX(toolbar), button, TRUE, TRUE, 0);

    app.draw_mode_button = gtk_button_new_with_label("Draw 1");
    g_signal_connect(app.draw_mode_button, "clicked", G_CALLBACK(draw_mode_cb), NULL);
    gtk_box_pack_start(GTK_BOX(toolbar), app.draw_mode_button, TRUE, TRUE, 0);
